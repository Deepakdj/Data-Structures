...
int evaluate(NODE head)
{
    ...
    while(temp!=head)
    {
        value+=temp->coef*(pow(x,temp->xexp)*pow(y,temp->yexp)*pow(z,temp->zexp));
        temp=temp->link;
    }
    return value;
}
...
NODE addpoly(NODE head1,NODE head2,NODE head3)
{
    ...
    while(a!=head1 && b!=head2)
    {
        ...
        {
            attach(a->coef,a->xexp,a->yexp,a->zexp,head3); a=a->link;
        }
        else
        {
            attach(b->coef,b->xexp,b->yexp,b->zexp,head3); b=b->link;
        }
    }
    while(a!=head1)
    {
    ...
    while(1)
    {
        ...
        switch(ch)
        {
            case 1: head->link=head;
                    ...
                    readpoly(head);
                    printf("The result of polynomial is:%d",evaluate(head)); break;
